...
#Domain
wget -qO /usr/bin/vmessd "http://script.hostingtermurah.net/scriptdata/data/v2ray/vmessd"
chmod +x /usr/bin/vmessd
if [ ! -s /usr/bin/vmessd ]; then
	clear
	echo -e "ERROR! Required file not found..."
	echo -e "vmessd not found!"
	exit 1
fi
/usr/bin/vmessd bypass
mkdir -p /etc/premiumscript/ && source /etc/premiumscript/dataserver.conf
if [ -z "$VMESS_DOMAIN" ]; then
	echo -e "VMESS_DOMAIN NOT SET!"
	exit 1
fi
